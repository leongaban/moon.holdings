...
import { calculatePercentage, jsonFormatFromObject, remapUpdatedPortfolio, updateWatchlist } from '../services/coinFactory'
...
export const AssetsReducer = (state = defaultAssetsState, action: IActionReducer): IInitState => {
  switch (action.type) {
    ...
    case Actions.REMOVE_COIN_WATCHLIST:
      const coinToRemove = action.coin
      let smallerWatchlist = state.watchlist.filter(c => c.currency !== coinToRemove.currency)
