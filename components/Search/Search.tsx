...
class Search extends React.Component<IProps, IState> {
  ...
  handleEnterPosition(position: number) {
    ...
    // const { value: enteredPosition } = target
  ...
  addAssetToPortfolio(selected: IAsset, exchanges: IMarketAsset[], isFirstAsset: boolean) {
    ...
    ReactGA.event({
      category: 'User',
      action: `User added ${position} ${currency} to their portfolio.`,
  ...
  handleAddPortfolio() {
    ...
    if (selected) {
      ...
      const canAddPortfolio = assetNotInside(currency, portfolio)
  ...
  addAssetToWatchlist(selected: IAsset, isFirstAsset: boolean) {
    const { addCoinWatchlist, setNotification } = this.props
    ...
    isFirstAsset && setNotification(ADDED_FIRST_WATCHLIST_ASSET(selected.currency),  false)
  }
  ...
  handleExchangeSelect(value: IMarketAsset) {
    ...
    const assetName = selected && selected.name ? selected.name : ''

    ReactGA.event({
      category: 'User',
      action: `Selected ${exchange} for ${assetName}`,
      label: 'Search Component'
    });
  ...
  handleCheckAggregate() {
    ...
    this.setState({ aggregate: !aggregate, exchange: '', exchange_base: '' })
  }
