...
import { MOON_PORTFOLIO, CURRENCY } from '../shared/constants/copy'
...
export const getExchangePrice = (selectedExchange: string, exchanges: IMarketAsset[]) => {
  const assetExchange = exchanges.filter(({ exchange }) =>
    exchange === selectedExchange.toLowerCase())[0]
...
export const calculatePercentage = (type: string, portfolio: IAsset[], coin?: IAsset) => {
  ...
  const newPortfolio = portfolio.map((coin) => {
    ...
    if (type === Actions.ADD_COIN_PORTFOLIO || type === Actions.REMOVE_COIN_PORTFOLIO) {
      const { value } = coin
      if (value) coinValue = value ? value : 0
    }
    else if (type === Actions.UPDATE_COIN_PORTFOLIO) {
...
export const updateWatchlist = (coin: IAsset, watchlist: IAsset[]) => R.concat(watchlist, [coin]);
...
// * Checks if asset is in Portfolio.
export const assetNotInside = (currency: string, list: IAsset[]) => R.not(R.find(R.propEq(CURRENCY, currency))(list))

// * Checks if the asset is a USD base asset.
export const usdBaseCurrencyCheck = (asset: IAsset) => {
  const isUSD = R.propEq(CURRENCY, 'USD')
  const isUSDT = R.propEq(CURRENCY, 'USDT')
  const isUSDC = R.propEq(CURRENCY, 'USDC')
  const isStableCoin = R.anyPass([isUSD, isUSDT, isUSDC])
  return isStableCoin(asset)
}
