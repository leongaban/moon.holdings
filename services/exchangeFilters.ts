...
export const extractExchangePrice = (asset: IAsset, markets: IGetMarketsRes) => {
  ...
  const useExchange = (base: string) => {
    switch (base) {
      case 'USD': case 'USDT': case 'USDC': return markets['market'+base]
  ...
  const exchangeFiltered = exchangeBase && exchangeBase.filter((market: IMarketAsset) => {
    if (market && market.exchange === exchange) {
      return market.base === currency && market.quote === base
    }
  })

  const exchangeObject: IMarketAsset | undefined = exchangeFiltered ? R.head(exchangeFiltered) : undefined
